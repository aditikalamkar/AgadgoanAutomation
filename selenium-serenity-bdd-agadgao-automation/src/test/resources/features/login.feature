Feature: Devotee App Login

  # Envirionment to pass from Global Properties file and will be handled in logic.
  @login @MF-001
  Scenario Outline: Devotee_<customerType>_logins_with_<scenario>
    Given "<customerType>" is valid Devotee user
  ...
  @login @MF-001
  Scenario Outline: Devotee_<customerType>_recieves_warning_for_account_to_get_locked_after_<noOfTry>_worng_password_attempts
    Given "<customerType>" is valid Devotee user
    When user logins with credential from "<TC_ID>"
  ...
  @login @MF-001
  Scenario Outline: Devotee_<customerType>_should_able_to_reset_password
    Given "customerType" is valid Devotee user
